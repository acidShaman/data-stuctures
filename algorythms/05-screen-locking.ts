...
function countPatternsFrom(firstPoint, length) {
    ...
    // Defining the jumps that require an intermediary point
    ...
    const dfs = (current, remaining, visited) => {
      ...
      for (let next = 1; next <= 9; next++) {
        if (!visited[next] && (jumps[current][next] === 0 || visited[jumps[current][next]])) {
    ...
    const startPoint = firstPoint.charCodeAt(0) - 'A'.charCodeAt(0) + 1;
    const visited = Array(10).fill(false);
    return dfs(startPoint, length - 1, visited);
  }
  
  // Example usage:
  console.log(countPatternsFrom('C', 2)); // Output: 5
  console.log(countPatternsFrom('A', 3)); // Output: 31