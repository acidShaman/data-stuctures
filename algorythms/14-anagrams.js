...
function listPosition(word) {
    ...
    const countSmallerInRight = (char, freq, length) => {
        let count = 0;
        for (let key in freq) {
            if (key < char && freq[key] > 0) {
                count += factorial(length - 1) / Object.values(freq).reduce((acc, val) => acc * factorial(val), 1);
            }
        }
        return count;
    };
    ...
    let freq = charCount(word);

    for (let i = 0; i < length; i++) {
        let char = word[i];
        position += countSmallerInRight(char, freq, length - i);
        freq[char]--;
    }
...
// Example usage:
console.log(listPosition("ABAB")); // Output: 2
console.log(listPosition("AAAB")); // Output: 1
console.log(listPosition("BAAA")); // Output: 4
console.log(listPosition("QUESTION")); // Output: 24572
console.log(listPosition("BOOKKEEPER")); // Output: 10743
