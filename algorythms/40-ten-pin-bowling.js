...
function bowlingScore(frames) {
    ...
    // Convert frames into rolls
    for (let frame of frameArr) {
        if (frame === 'X') {
            rolls.push(10);
    ...
    // Add extra rolls for the 10th frame
    if (frameArr.length === 10 && frameArr[9].length === 3) {
        let lastFrame = frameArr[9];
        for (let char of lastFrame) {
            if (char === 'X') {
                rolls.push(10);
            } else if (char === '/') {
                rolls.push(10 - parseInt(lastFrame[lastFrame.indexOf('/') - 1]));
            } else {
...
// Example usage:
console.log(bowlingScore('X X 9/ 80 X X 90 8/ 7/ 44')); // Output: 171
console.log(bowlingScore('X X X X X X X X X XXX')); // Output: 300
console.log(bowlingScore('54 72 44')); // Output: 27
console.log(bowlingScore('5/ 54 44 X X 5/ 7/ X 23 6/')); // Output: 154