...
function isInteresting(number, awesomePhrases) {
    if (number < 100) {
      return checkNextTwoInteresting(number, awesomePhrases);
    }
  ...
  function checkNextTwoInteresting(number, awesomePhrases) {
    if (number + 1 >= 100 && isNumberInteresting(number + 1, awesomePhrases)) {
      return 1;
    }
    if (number + 2 >= 100 && isNumberInteresting(number + 2, awesomePhrases)) {
      return 1;
    }
    return 0;
  }
  
  function isNumberInteresting(number, awesomePhrases) {
    return (
  ...
  function followedByZeros(number) {
    ...
    return str.slice(1).split('').every(digit => digit === '0');
  }
  ...
  function isIncrementing(number) {
    const str = number.toString();
    const incrementingSequence = '1234567890';
    return incrementingSequence.includes(str);
  }
  
  function isDecrementing(number) {
    const str = number.toString();
    const decrementingSequence = '9876543210';
    return decrementingSequence.includes(str);
  }
